...
function BrowserBroadcaster() {
  ...
  const [mediaAccessError, setMediaAccessError] =
    useState<ErrorMessageEnum | null>(null);
  const [publishSuccess, setPublishSuccess] = useState(false);
  const [useDisplayMedia, setUseDisplayMedia] = useState<
    'Screen' | 'Webcam' | 'None'
  >('None');
          ...
          sender.getStats().then((stats) => {
            stats.forEach((report) => {
              ...
              if (report.type === 'candidate-pair') {
                const signalIsValid =
                  report.availableIncomingBitrate !== undefined;
                badSignalCountRef.current = signalIsValid
                  ? 0
                  : badSignalCountRef.current + 1;

                if (badSignalCountRef.current > 2) {
                  setHasSignal(() => false);
                } else if (
                  badSignalCountRef.current === 0 &&
  ...
  return (
    <div className='container mx-auto'>
      ...
      {hasPacketLoss && (
        <PlayerHeader headerType={'Warning'}>
          {' '}
          WebRTC is experiencing packet loss
        </PlayerHeader>
      )}
      {publishSuccess && (
